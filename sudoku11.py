...
def brute_force(pzl, invalid_dct):
    ...
    best_sym, positions = optimal_sym(pzl, len(choices))  # optimal symbol based on number of possible positions

    if len(positions) < len(choices):  # traverse whichever structure has (less possibilities <=> less chance of error)
    ...
    for i, choice in enumerate(choices):  # same process as just above
...
def optimal_sym(pzl, len_choices):  # finds symbol with least possible positions within its constraint set
    ...
    for cs in CONSTRAINTS:                             # go through each constraint set
        for sym in SYMBOL_SET - {pzl[i] for i in cs}:  # go through each unfilled symbol in the constraint set
            ...
            if curr_len == 0 or curr_len == 1:                # pzl invalid or exactly one valid choice
