...
def is_invalid(pzl, pos):
    # pzl is invalid if last placed item results in a constraint set having duplicates
    if pos == -1:
        return False
...
def brute_force(pzl, pos, invalid_dct):
    ...
    for choice in SYMBOLS - invalid:
        ...
        update_invalid(sub_pzl, invalid_dct, best_pos)
        b_f = brute_force(sub_pzl, best_pos, invalid_dct)
        if b_f:
            return b_f
        invalid_dct = copy_invalid
    return ''


def un_change(pzl, invalid_dct, pos, changed):
    invalid_dct[pos] = changed
    for i in NEIGHBORS[pos]:
        if i in invalid_dct and pzl[pos] not in [pzl[n] for n in NEIGHBORS[i]]:
            try: invalid_dct[i].remove(pzl[pos])
            except KeyError: pass


def update_invalid(pzl, invalid_dct, pos):
    invalid_dct.pop(pos)
    for i in NEIGHBORS[pos]:
        if i in invalid_dct:
            invalid_dct[i].add(pzl[pos])
...
def set_globals(pzl):
    ...
    global PW, SYMBOLS, NEIGHBORS, BW, STATS_COUNT
