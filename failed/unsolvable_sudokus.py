...
def is_invalid(pzl, last_filled_pos):
    ...
    for group in GROUPS:
        ...
        for i in group:
            if i == '.':
                has_dot = True
                if len({pzl[c] for c in CONSTRAINTS[i]})-1 != len(SYMBOLS):
                    possibilities.append(True)
                    break
        if has_dot and not possibilities:
            return True
...
def brute_force(pzl, last_filled_pos):
    if is_invalid(pzl, last_filled_pos):
        return ''
...
def set_globals(pzl):
    ...
    global N, SYMBOLS, CONSTRAINTS, GROUPS, BLOCK_SIZE
    ...
    GROUPS = sorted([*{tuple(
        i // (N * BLOCK_SIZE) * (N * BLOCK_SIZE) + i % N - i % BLOCK_SIZE + k + j for k in range(BLOCK_SIZE) for
        j in range(0, BLOCK_SIZE * N, N)) for i in range(N * N)}]) + \
        [[i % N + r * N for r in range(N)] for i in range(N)] + \
        [[c + i * N for c in range(N)] for i in range(N)]
