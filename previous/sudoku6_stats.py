...
def brute_force(pzl, invalid_dct):
    if '.' not in pzl:
        update_stats("bf return 1")
        return pzl

    best_pos, invalid = optimal_pos(invalid_dct)
    choices = SYMBOLS - invalid
    update_stats(f"choice count {len(choices)}")
    for choice in choices:
        sub_pzl = pzl[:best_pos] + choice + pzl[best_pos + 1:]
        removed, added_invalid = update_invalid_dct(sub_pzl, best_pos, invalid_dct)
        b_f = brute_force(sub_pzl, invalid_dct)
        if b_f:
            update_stats("bf return 2")
            return b_f
        revert_invalid_dct(choice, best_pos, added_invalid, removed, invalid_dct)
    update_stats("bf return 3")
    return ''


def update_invalid_dct(pzl, pos, invalid_dct):
    removed = invalid_dct[pos]
    added_invalid = []
    invalid_dct.pop(pos)
    for n in NEIGHBORS[pos]:
        if pzl[n] == '.' and pzl[pos] not in invalid_dct[n]:
            invalid_dct[n].add(pzl[pos])
            added_invalid.append(n)
    return removed, added_invalid


def revert_invalid_dct(choice, best_pos, added_invalid, removed, invalid_dct):
