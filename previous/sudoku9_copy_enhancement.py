...
def brute_force(pzl, invalid_dct):
    ...
    best_sym, positions = optimal_sym(pzl)
...
def optimal_sym(pzl):
    best_sym, min_possible_positions = pzl[0], [i for i in range(PW)]
    for constraint_set in CONSTRAINTS:
        for sym in SYMBOL_SET - {pzl[i] for i in constraint_set}:
            # possible_positions = [c for c in cs if pzl[c] == '.' and sym not in [pzl[n] for n in NEIGHBORS[c]]]
            possible_positions = []
            ...
            if len(possible_positions) < len(min_possible_positions):
                best_sym, min_possible_positions = sym, possible_positions
